    ...
    async function loadCustomers() {
        mainContent.innerHTML = `
            ...
            <div id="orderDetailForm">
                ...
                <input type="text" id="order_phone" placeholder="Nhập số điện thoại">
        ...
        document.getElementById('addOrderDetail').onclick = function() {
            ...
            // Kiểm tra đã có sản phẩm này với số điện thoại này chưa
            const existing = orderDetails.find(item => item.productId === productId && item.phone === phone);
            if (existing) {
                existing.quantity += quantity;
            } else {
                orderDetails.push({ productId, productName, unitPrice, quantity, phone });
            }
        ...
        function renderOrderDetailTable() {
            ...
            orderDetails.forEach((item, idx) => {
                ...
                // Hiển thị ID đơn hàng nếu đã có (sau khi lưu), còn chưa thì để trống
                row.innerHTML = `
